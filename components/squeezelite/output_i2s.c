...
void output_init_i2s() {
    ...
    // do we want stats
    if (platform->has_services && platform->services.statistics) {
...
static void output_thread_i2s(void* arg) {
    ...
    while (running) {
        ...
        if (i2s_config.sample_rate != output.current_sample_rate) {
            ...
            if (synced) {
                /*
                   ...
                   synced usleep(((DMA_BUF_COUNT - 1) * DMA_BUF_LEN * BYTES_PER_FRAME * 1000) /
                   44100 * 1000); discard = DMA_BUF_COUNT * DMA_BUF_LEN * BYTES_PER_FRAME;
                */
            }
            i2s_config.sample_rate = output.current_sample_rate;
        ...
        if (spdif.enabled) {
            ...
            bytes = 0;
            // need IRAM for speed but can't allocate a FRAME_BLOCK * 16, so process by smaller
            // chunks
            while (count < oframes) {
...
/*
 SPDIF is supposed to be (before BMC encoding, from LSB to MSB)
    ...
    PPPP AAAA  SSSS SSSS  SSSS SSSS  SSSS VUCP (P=preamble, A=auxiliary, S=sample-20bits, V=valid,
 U=user data, C=channel status, P=parity) After BMC encoding, each bit becomes 2 hence this becomes
 a 64 bits word. The parity is fixed by changing AAAA bits so that VUPC does not change. Then then
 trick is to start not with a PPPP sequence but with an VUCP sequence to that the 16 bits samples
