...
```
docker pull espressif/idf:v5.5
```
Then run the container interactively :
```
for windows:
docker run -v %cd%:/project -w /project -it espressif/idf:v5.5
for linux:
docker run -it -v `pwd`:/project -w /project espressif/idf:v5.5
```
...
**Use the esp-idf 5.5 https://github.com/espressif/esp-idf/tree/release/v5.5 ** for ESP32, ESP32-S3, and other supported targets. You should clone recursively the whole branch (at the version you need) `git clone -b v5.5 https://github.com/espressif/esp-idf --recursive` and run the installer (`install.bat [esp32[,esp32s3]]` from there. Some Windows version (at least) have now a SSL certificate issue. You can workaround this by editing idf-tools.py and adding the following under ìmport ssl`
```
import ssl
